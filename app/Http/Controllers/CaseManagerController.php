...
class CaseManagerController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            'postal_code' => ['required', 'string', 'regex:/^\S+$/', 'unique:cases,postal_code'],
        ], [
            'postal_code.unique' => 'Postal code must be unique across all cases.',
        ]);
    ...
    public function edit(CaseFile $caseFile, Request $request): Response
    {
        ...
        $logs = DB::table('activity_logs')
            ...
            ->where('activity_logs.target_type', 'case')
            ->where('activity_logs.target_id', $caseFile->id)
    ...
    public function updateDetails(Request $request, CaseFile $caseFile): RedirectResponse
    {
        ...
        $validated = $request->validate([
            'postal_code' => ['required', 'string', 'regex:/^\S+$/', 'unique:cases,postal_code,' . $caseFile->id],
    ...
    /**
     * Toggle attention flags (attention, mail, doc) for the given case.
     */
    public function toggleAttention(Request $request, CaseFile $caseFile, string $type): RedirectResponse
    {
        ...
        if (! in_array($type, ['attention', 'mail', 'doc'])) {
